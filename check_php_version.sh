...
EXT_GITHUB_REPO="DigitalCyberSoft/php-quic"
...
if check_ext_version; then
    ...
    VERSIONS_CHANGED=1
    CHANGE_SUMMARY="  Extension version updated (release reset to 1)\n"
fi
echo
    ...
    if [ "$MODE" = "update" ]; then
        if [ $DRY_RUN -eq 1 ]; then
            ...
            if [ $EXT_VERSION_CHANGED -eq 0 ]; then
                ...
                echo "Skipping release bump (already reset by extension update)"
            fi
